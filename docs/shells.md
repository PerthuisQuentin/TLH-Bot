...
It is the only `UpgradeKind.CUSTOM` upgrade. It produces nothing, so `computeIncome` — which matches ADDITIVE and MULTIPLICATIVE and falls through on anything else — skips it. Its whole effect is that its level is above 0.
